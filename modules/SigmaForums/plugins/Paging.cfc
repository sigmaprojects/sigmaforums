...
<!------------------------------------------- PRIVATE ------------------------------------------->	
	...
	--->
	<cffunction name="queryStringToStruct" access="private" output="false" returntype="struct" hint="Converts a URL query string to a structure.">
		...
		<cfset var valuei = "" />
		<cfset var qsarray = "" />
		<cfset var qs = arguments.queryString />
		<cfset qsarray = listToArray(qs, "&") />
		<cfloop from="1" to="#ArrayLen(qsarray)#" index="i">
			<cfset pairi = qsarray[i] />
			<cfset keyi = listFirst(pairi,"=") />
			<cfset valuei = urlDecode(listLast(pairi,"=")) />
			<cfif structKeyExists(qStruct,keyi)>
				<cfset qStruct[keyi] = listAppend(qStruct[keyi],valuei) />
			<cfelse>
				<cfset structInsert(qStruct,keyi,valuei) />
			</cfif>
		</cfloop>
		<cfreturn qStruct />
	</cffunction>
	...
	--->
	<cffunction name="StructToQueryStringT" access="private" output="false" hint="Converts a structure to a URL query string."> 
		<cfargument name="qStruct" type="struct" required="true" /> 
		<cfargument name="delim1" type="string" default="=" />
		<cfargument name="delim2" type="string" default="&" />
		<cfset var qstr = "" />
		<cfset var key = "" />
		<cfloop collection=#arguments.qStruct# item="key"> 
			 <cfset qstr = ListAppend(qstr, URLEncodedFormat(LCase(key)) & arguments.delim1 & URLEncodedFormat(arguments.qStruct[key]), arguments.delim2) />
		</cfloop>
		<cfreturn qstr />
	</cffunction>
